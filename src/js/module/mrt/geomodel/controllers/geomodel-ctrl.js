angular.module('MRT').controller('GeoModelWeightCtrl', ['$scope', '$filter', '$stateParams', '$modal', '$log', 'geoModelService', 'geoGroupService', function($scope, $filter, $stateParams, $modal, $log, geoModelService, geoGroupService) {
		...
		$scope.open = function(size) {
			var modalInstance = $modal.open({
				templateUrl: 'indicator-update.html',
	...
	}]).controller('GeoModelIndicatorUpdateCtrl', ['$scope', '$filter', '$modalInstance', 'geoindicators', 'params', 'geoIndicatorGroupService', 'geoIndicatorService', 'geoModelService', function($scope, $filter, $modalInstance, geoindicators, params, geoIndicatorGroupService, geoIndicatorService, geoModelService) {
		$scope.data = {};
		...
		var data = geoIndicatorService.getGeoIndicatorsFromCache($scope.data.params);
		return false;
		$scope.data.list = data.geoindicators;

		$scope.sortList();
		$scope.selectGeoIndicators();
		...
		$scope.submitForm = function(isValid) {
			...
			angular.forEach($scope.data.list, function(item, key) {
				if (!angular.isUndefined(item.selected) && item.selected === true) {
					var modelParameterIndicator = {};
					modelParameterIndicator.parameter_id = $scope.data.parameter.id;
					modelParameterIndicator.indicator_id = item.id;

					var found = $filter('filter')($scope.data.geoindicators, {selected: true, geoindicator_id: item.id}, true);

					if (found.length) {
						modelParameterIndicator.weight = found[0].weight;
						modelParameterIndicator.relevance_sort = found[0].relevance_sort;
					} else {
						modelParameterIndicator.weight = !angular.isUndefined(item.weight) ? item.weight : 0;
						modelParameterIndicator.relevance_sort = !angular.isUndefined(item.relevance_sort) ? item.relevance_sort : 'ASC';
					}

					$scope.data.modelParameterIndicators.push(modelParameterIndicator);

				}
			});
			console.log($scope.data.modelParameterIndicators);
			return false;
		...
		$scope.selectGeoIndicators = function() {
			angular.forEach($scope.data.geoindicators, function(geoindicator, key) {
				if (geoindicator.selected === true) {
					$scope.select(geoindicator.geoindicator_id);
				} else {
					$scope.deselect(geoindicator.geoindicator_id);
				}
			});
